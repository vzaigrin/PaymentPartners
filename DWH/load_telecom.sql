...
BEGIN
    ...
    SET total = (SELECT count(*) FROM PP.STG_TELECOM);
    SET good = (SELECT count(*) FROM PP.ODS_TELECOM);

    INSERT INTO PP.DM_LOADS (partner_name, period_name, period_year, period_month, filename, load_ts, total, good, bad)
    VALUES ('telecom', FORMAT("%4d-%02d", pyear, pmonth), pyear, pmonth, fname, loadts, total, good, total - good);
