...
- ADD_PARTNER() - добавление ноаого партнёра
...
При добавлении в систему нового партнёра для него должны быть созданы таблицы в областях Stage и ODS, и процедура перегрузки данных в область DDS.
...
Скрипт **genall.sh** генерирует код для каждого партнёра.

Так как разграничение доступа к данным в BigQuery осуществляется на уровне наборов данных, для каждого партнёра создаётся ещё и свой набор данных с названием, совпадающем с именем партнёра. Эти наборы содержат представления (View) с витринами, содержащие данные только этого партнёра.
