...
Скрипт **gen2019.sh** генерирует данные за весь 2019 год для всех шаблонов с 100000 записей с 10% ошибок в каждом месяце для каждого партнёра. Скрипт создаёт папку *hist* для данных за первые 10 месяцев, и папку *12* для данных за декабрь.
...
При загрузки файлов со справочниками в систему срабатывает скрипт [Cloud Functions](https://cloud.google.com/functions/) **load_dicts**. Файлы со справочниками должны называться так же как называются таблицы справочников, и должны содержать весь справочник.

Для обработки файлов с реестрами по расписанию (16-го числа каждого месяца в 0:0) запускается процедура **load_data**.

Оба скрипта:
- загружают данные из файла в область Stage
- вызывают процедуру перегрузки данных в области ODS и DDS
- удаляют файл

Исторические данные (реестры за прошлые периоды) в Cloud Storage не выгружаются. Для их загрузки используется скрипт **load_hist_data**. Он также загружает данные из файлов в область Stage и вызывает процедуру перегрузки данных в области ODS и DDS, но не удаляет файлы.
...
- V_DM_LOADS_TAXI - отчёты о загрузках данных для партнёра Taxi
- V_DM_LOADS_TELECOM - отчёты о загрузках данных для партнёра Telecom
- V_DM_LOADS_CINEMA - отчёты о загрузках данных для партнёра Cinema
- V_DM_LOADS_RETAIL - отчёты о загрузках данных для партнёра Retail
...
Для заполнения данными таблиц в область ODS и DDS в DWH созданы специальные процедуры:
- LOAD_PARTNERS() - данные о партнёрах
- LOAD_BINS() - данные о bins
- LOAD_PRIVILEGE() - данные о привилегиях
- LOAD_TAXI() - данные партнёра Taxi
- LOAD_TELECOM() - данные партнёра Telecom
- LOAD_CINEMA() - данные партнёра Cinema
- LOAD_RETAIL() - данные партнёра Retail
